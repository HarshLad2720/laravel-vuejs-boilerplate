...
HTTP.interceptors.request.use(
    function (config) {
        if (config.url == "/api/v1/login" || config.url.includes("forgot-passsword") || config.url.includes("reset-password") || config.url.includes("register") || (config.method == 'get' && (config.url.includes("countries") || config.url.includes("states") || config.url.includes("cities") || config.url.includes("hobbies")))) {
            return config;
        }
